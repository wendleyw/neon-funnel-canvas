...
import { useComponentTemplates } from '@/features/templates/hooks/useComponentTemplates';
import { MARKETING_COMPONENT_TEMPLATES } from '../../../data/componentTemplates';
import { ChevronDown, Plus, Clock, Target, Zap, RefreshCw } from 'lucide-react';
  ...
  const ComponentCard = ({ template }: { template: ComponentTemplate }) => {
    ...
    return (
      <div
        ...
        className="group relative component-card-enhanced flex flex-col gap-3 p-4 rounded-xl cursor-pointer"
      >
    ...
    return (
      <div className="mb-6">
        ...
        <button
          ...
          className="w-full flex items-center justify-between p-3 rounded-xl category-header-enhanced transition-all duration-200 group"
        >
  ...
  if (error) {
    return (
      <div className="h-full flex flex-col">
        <div className="p-4 border-b border-gray-800/50">
          <h2 className="font-bold text-white text-lg">Components</h2>
          <p className="text-xs text-red-400 mt-1">Error loading templates</p>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Drag to canvas or click to add • {templates.length} total
            </p>
          </div>
        </div>
      </div>
